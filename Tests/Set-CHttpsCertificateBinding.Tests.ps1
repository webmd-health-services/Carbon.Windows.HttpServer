...
function Test-ShouldCreateNewHttpsCertificateBinding
{
    $appID = '0e8a659e-8034-4ab1-ab82-dcb0f5e90bfd'
    $ipAddress = '74.32.80.43'
    $port = '3847'
    $binding = Set-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port -ApplicationID $appID -Thumbprint $cert.Thumbprint
    try
    {
        Assert-Null $binding
...
function Test-ShouldReturnBinding
{
    ...
    $ipAddress = '74.32.80.43'
    $port = '3847'
    $binding = Set-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port -ApplicationID $appID -Thumbprint $cert.Thumbprint -PassThru
...
function Test-ShouldUpdateExistingHttpsCertificateBinding
{
    ...
    $binding = Set-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port -ApplicationID $appID -Thumbprint $cert.Thumbprint
    Assert-Null $binding
    $binding = Set-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port -ApplicationID $newAppID -Thumbprint $cert.Thumbprint
    Assert-Null $binding
    $binding = Get-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port
    try
    {
...
function Test-ShouldSupportShouldProcess
{
    ...
    $port = '4782'
    $binding = Set-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port -ApplicationID $appID -Thumbprint $cert.Thumbprint -WhatIf
    Assert-Null $binding
    try
    {
        Assert-Null (Get-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port)
    }
    finally
    {
        Remove-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port
...
function Test-ShouldSupportShouldProcessOnBindingUpdate
{
    ...
    $binding = Set-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port -ApplicationID $appID -Thumbprint $cert.Thumbprint
    Assert-Null $binding
    $binding = Set-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port -ApplicationID $newAppID -Thumbprint $cert.Thumbprint -WhatIf
    Assert-Null $binding
    $binding = Get-CHttpsCertificateBinding -IPAddress $ipAddress -Port $port
    try
    {
